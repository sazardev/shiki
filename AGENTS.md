...
- There are **~292 `#[test]`s** (134 in `shiki-core`, 17 `shiki-config`, 128 `shiki-tui`, 13 `shiki-cli`) — CLAUDE.md's "almost no tests yet" paragraph is stale. They are all inline `#[cfg(test)]` modules inside source files (no `tests/` dirs, no `#[ignore]`, no fixture setup). `cargo test --workspace` is green.
