...
install_manifest() {
  ...
  if [[ -f "$dest" && "$EXT_ID" != "__REPLACE_WITH_EXTENSION_ID__" ]]; then
    if command -v python3 >/dev/null 2>&1; then
      dest="$dest" EXT_ID="$EXT_ID" HOST_BIN="$HOST_BIN" python3 -c "
...
" && return 0
  ...
  # Fallback sed with safe delimiter (|) for paths with /
...
if [[ -n "${DEST_DIR_FIREFOX:-}" ]]; then
  ...
  MANIFEST_TEMPLATE="$MANIFEST_TEMPLATE" HOST_BIN="$HOST_BIN" DEST_DIR_FIREFOX="$DEST_DIR_FIREFOX" python3 -c "
import json, pathlib, os
src = pathlib.Path(os.environ['MANIFEST_TEMPLATE'])
data = json.loads(src.read_text())
data['path'] = os.environ['HOST_BIN']
...
os.makedirs(os.environ['DEST_DIR_FIREFOX'], exist_ok=True)
open(os.path.join(os.environ['DEST_DIR_FIREFOX'], 'com.shiki.native.json'),'w').write(json.dumps(data, indent=2))
print(f\"    installed firefox manifest: {os.environ['DEST_DIR_FIREFOX']}/com.shiki.native.json\")
"
fi
